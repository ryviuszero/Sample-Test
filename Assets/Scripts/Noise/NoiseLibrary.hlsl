...
int3 GetBlockMax(int blockSize, int3 blockCoord) {
    int3 blockMax;
    blockMax.x = (blockCoord.x + 1) * blockSize ;
    blockMax.y = (blockCoord.y + 1) * blockSize;
    blockMax.z = (blockCoord.z + 1) * blockSize;
    return blockMax;
}

int3 PixelCoordBlockCoord(int blockSize, int3 pixelCoord) {
    int3 blockCoord;
...
float3 GetRandom3To3_Remapped(float param, float randomSeed) {
    float3 value = GetRandom3To_Raw(param, randomSeed);
    value.x = (value.x + 1) / 2;
    value.y = (value.y + 1) / 2;
    value.z = (value.z + 1) / 2;
    return value;
